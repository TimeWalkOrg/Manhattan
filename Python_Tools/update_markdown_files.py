...
def update_markdown_file(file_path, images_folder):
    ...
    img_tag_urls = re.findall(r'<img.*?src="(https?://.*?)".*?>', content)
    ...
    for img_url in image_urls:
        ...
        if local_image_path:
            new_path = os.path.relpath(local_image_path, start=os.path.dirname(file_path))
...
os.system(f"cd {local_dir} && git commit -m 'Update image paths'")

# Step 8: Pull latest changes from the remote repository
pull_result = os.system(f"cd {local_dir} && git pull new-wiki master --allow-unrelated-histories")

# Step 9: If conflicts occur, inform the user
if pull_result != 0:
    print("Merge conflicts detected. Please resolve them manually and then run the script again.")
else:
    # Step 10: Push changes to the new wiki repository
